...
(function (BP3D) {
    ...
    (function (Three) {
        Three.Skybox = function (scene) {
            var scope = this;
            var scene = scene;
            var topColor = 0xC0C0C0; //0xffffff;//0xD8ECF9
            var bottomColor = 0xC0C0C0; //0xf9f9f9;//0x565e63
            var verticalOffset = 500;
            var sphereRadius = 6000;
...
(function (BP3D) {
    var Three;
    (function (Three) {
        Three.Human = function (scene, model) {
            // var meshes = [];
            var mixers = [];
            var items = [];
            var doors = [];
            ...
            function init() {
                ...
                var fire = new Three.Fire(scene, model);
            }
            function addModelToScene(geometry, materials) {
                ...
                for (var j = 0; j < steps[0].length; j++) {
                    if (steps[0][j].agent != undefined) {
                        ...
                        var room = whichRoom(steps[0][j].room);
                        var roomNumber = getRoom(room.x, room.y);
                    ...
                    if (stepArr && stepArr.length != 0) {
                        for (var i = 0; i < stepArr.length; i++) {
                            if (stepArr[i].agent != undefined) {
                                if (scene.meshes[stepArr[i].agent] == undefined) {
                                    ...
                                    if (stepArr[i].moveTo != undefined && stepArr[i].toStep != undefined) {
                                        var time = (stepArr[i].toStep - step) * scene.stepTime;
                                        if (type == 0) {
                            ...
                            if (stepArr[i].light != undefined) {
                                var room_2 = whichRoom(stepArr[i].room);
                                var roomNumber = getRoom(room_2.x, room_2.y);
                            ...
                            if (stepArr[i].video != undefined) {
                                ...
                                var video = new Three.Video(scene, model, getRoom(roomCoordinates.x, roomCoordinates.y));
                            }
...
(function (BP3D) {
    ...
    (function (Three) {
        Three.Main = function (model, element, canvasElement, opts) {
            ...
            var meshes = [];
            var mixers = [];
            var human = new Three.Human(scene, model);
            var video;
            var imageContext;
            var textureVideo;
            var paused = 1;
